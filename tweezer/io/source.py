...
class TxtSourceMpi():
    ...
    def convert_header(self, header, columnUnits=None):
        ...
        Returns:
            :class:`tweezer.container.MetaDict` and :class:`tweezer.container.UnitDict`
        """
    ...
    def write_data(self, meta, units, data, target=None):
        """
        ...
        with target.open(mode='w') as f:
            f.write(json.dumps(OrderedDict([('meta', meta), ('units', units)]), indent=4))
            f.write("\n\n#### DATA ####\n\n")

        data.to_csv(path_or_buf=target.__str__(), sep='\t', mode='a', index=False)
    ...
    def rename_key(self, meta, units, oldKey, newKey):
        ...
        Args:
            meta (:class:`tweezer.container.MetaDict`): meta dictionary
            units (:class:`tweezer.container.UnitDict`): units dictionary
