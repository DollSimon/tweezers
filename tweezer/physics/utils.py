...
def mass_sphere(radius:float = 1000, density:float = 1e-21,
    ...
    Args:
        radius (float): Radius in [nm]. (Default: 1000 nm)
        density (float): Density, ϱ, in [g / nm^3]. (Default: 1e-21 g/nm^3)
        verbose (bool): Flag whether to show extra information. (Default: False)
        inferDensity (bool): If true inputs above 1e-5 are interpreted as g/cm^3. (Default: True)
    ...
    # check if density is provided in g/cm^3
    if inferDensity and density > 1e-5:
        print("Assuming that ϱ was provided in [g/cm^3]. Converting to g/nm^3")
        density /= 1e21
        print("ϱ is now: {} g/nm^3".format(density))
