...
def iterative_fitting(file, plt=False):
    ...
    for column in data:
        ...
        limits = 0.1
        step = 0.1
        D, fc, sigma, checkA = factors_from_data(column, limit=limits, plot=False)
        D, fc, sigma, check = factors_from_data(column, limit=limits+step, plot=False)

        while abs((check-checkA)/check)*100 > 1:

            checkB = checkA
            checkA = check
            D, fc, sigma, check = factors_from_data(column, limit=limits, plot=False)
            ...
            if check < checkA:
                ...
                step = -step*0.8
            limits = limits + step
        D, fc, sigma, check = factors_from_data(column, limit=limits, plot=plt)
        Ds.append(D)
        fcs.append(fc)
        sigmas.append(sigma)
                    
    return Ds, fcs, sigmas
    

D, fc, errors = iterative_fitting(pathFile, plt=True)
